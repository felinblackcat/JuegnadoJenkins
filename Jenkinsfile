node {
    ...
    stage("BuildTesting") {
        ...
        coverage = sh(script: "docker run -v /var/jenkins_home/workspace/CI_CDtest_main/app:/app juegnadojenkins pytest --junitxml=./test.xml --cov=. --cov-fail-under=90 | grep TOTAL| awk '{print \$4}' | tr -d %", returnStdout: true).trim()
        sh "ls app"
    }
    echo coverage
